import { remakeCharacter } from "./remake.js";
import {stockVendor} from "./stockvendor.js";
...
import {
	...
	negation,
	shrink
} from "./powerhandlers.js";

export const moduleId = 'super-noir';
export const powerPackName = moduleId + ".powers";
...
export async function getPackItem(packName, swid) {
		...
		fields: [
			{
                ...
                "options": [
                    ...
                    {"name": "Major (-2)", "value": -2}
                ],
				description: "<p>A Minor Limitation that prohibits the power from being used reduces its cost by 1 point. A Major Limitation (works about half the time) reduces its cost by 2.</p>"
			}
		]
	},
	'selective': {
...
export function formatRow(left, right, options = {}) {
	if (!right)
		return `<div style="display: table-row;">${left}</div>`;

	let ralign = options.ralign ? options.ralign : 'right';
	let lstyle = options.lstyle ? options.lstyle : '';
	return `<div style="display: table-row;">
		<div style="display: table-cell; text-align: left;${lstyle}">
			${left}
		</div>
